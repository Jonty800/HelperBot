...
namespace HelperBot {
    ...
    internal class Methods {
        ...
        public static string GetRandomStatString( Player player ) {
            ...
            if ( player == null )
                SendError( "HelperBot: Player cannot be null", MessageChannel.Global );
            byte Max = 13; //Max enum byte
            int StringID = new Random().Next( 0, Max );
            if ( Settings.ReleaseFlag == Flags.Debug )
                SendChat( StringID.ToString() );
            RandomStat Picked = ( RandomStat )StringID;
            if ( Settings.ReleaseFlag == Flags.Debug )
                SendChat( Picked.ToString() );
            switch ( Picked ) {
                ...
                case RandomStat.MostHours:
                    if ( Values.MostHours == null )
                        return "No players have achieved 1 whole hour on this server :(";
                ...
                case RandomStat.MostLogins:
                    if ( Values.MostLogins == null )
                        return "No one has the most log ins!!";
                ...
                case RandomStat.MostMessagesSent:
                    try {
                        if ( Values.MostMessagesSent == null )
                ...
                case RandomStat.MostPromoted:
                    if ( Values.MostPromoted == null )
                        return "No one has been promoted on this server yet :S";
                ...
                case RandomStat.MostTimesGotKicked:
                    if ( Values.MostTimesGotKicked == null )
                        return "No one has been kicked on this server";
        ...
        public static void RemoveTYPlayer( Player player ) {
            if ( Values.AwaitingThanks == null )
                return;
            if ( Values.AwaitingThanks.Count < 1 )
                return;
            lock ( Values.AwaitingThanks ) {
